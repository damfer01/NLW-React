...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
   ...
   function handleStartRecording() {
      ...
      if(!isSpeechRecognitionAPIAvailable){
         alert('navegador não suporta a API de gravação')
         return
      }
   ...
   return (
      <Dialog.Root>
         <Dialog.Trigger className='rounded-md flex flex-col bg-slate-700 text-left p-5  gap-3  overflow-hidden outline-none
               ...
               <form  className='flex-1 flex flex-col '>
                  ...
                  <div className=' flex flex-1  flex-col gap-3 p-5'>
                     ...
                     {showLdshowOnboarding ? (
                        <p className='text-sm leading-6 text-slate-400'>
                           Comece <button type='button' onClick={handleStartRecording} className='font-medium text-lime-400 hover:underline '> gravando uma nota</button> em audio ou se preferir <button type='button' onClick={handleStasrEditor} className='font-medium text-lime-400 hover:underline'>utilize apenas texto.</button>
                        </p>
                     ) : (
                        <textarea
                           autoFocus
